...
func (processor *RedisTaskProcessor) ProcessTaskSendVerifyEmail(ctx context.Context, task *asynq.Task) error {
	...
	if err != nil {
		// importance of delay async tasks. be careful of using asynq.SkipRetry
	...
	if err != nil {
		return fmt.Errorf("failed to create verify email database: %w", err)
	}

	subject := "Welcome to Simple Bank"
	verifyUrl := fmt.Sprintf("http://%s/v1/verify_email?email_id=%d&secret_code=%s", processor.config.Domain, verifyEmail.ID, verifyEmail.SecretCode)
